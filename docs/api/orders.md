...
Оба endpoint используют server-side сессию из cookie `turkiye_session`. Без действующей сессии API отвечает `401`. Клиент не передаёт `userId`, тип товара, цену или валюту. В ответах клиентского API нет внутреннего менеджерского признака `isProcessed`.
...
раскрывает внутренний менеджерский признак `isProcessed`.
