  ...
  return (
    <article className={cardStyles.serviceCard}>
      <Link
        ...
        className={cardStyles.serviceCardLink}
        href={`/services/${product.slug}`}
      >
        <div
          aria-label={product.title}
          className={cardStyles.serviceCardMedia}
          role="img"
          style={
            product.imageUrl
              ? {
        ...
        >
          <span className={cardStyles.serviceType}>
            {categoryName ?? productTypeLabel(product.type)}
          </span>
        </div>
        <div className={cardStyles.serviceCardContent}>
          ...
          <div className={cardStyles.serviceMeta}>
            <span>{productTypeLabel(product.type)}</span>
            <strong>
              <span className={cardStyles.priceLabel}>Цена</span>
...
export function CatalogBrowser() {
  ...
  const selectedDestinationSlug = searchParams.get("destination") ?? "";
  ...
  useEffect(() => {
    ...
    marketplaceApi
      .products(
        ...
        selectedDestinationSlug || undefined,
      )
  ...
  }, [selectedCategorySlug, selectedDestinationSlug]);
    ...
    <section aria-label="Каталог" className={styles.catalog}>
      <div className={styles.quickFilters}>
        ...
        <div className={styles.quickFilterList}>
          <button
            aria-pressed={!selectedCategorySlug && !selectedDestinationSlug}
      ...
      {products !== null && visibleProducts.length === 0 ? (
        <div className={styles.emptyState}>
          ...
          <p>Попробуйте выбрать другой раздел или изменить запрос.</p>
          <button
            onClick={() => {
              setSearch("");
              clearFilters();
