...
for (const [source, label] of [
  ...
  assert.match(source, /Оплата онлайн/, `${label} must show the payment label`);
  assert.match(source, /payment\/sbp-logo\.svg/, `${label} must reference the SBP badge`);
  assert.match(source, /payment\/mir-logo\.svg/, `${label} must reference the Mir badge`);
}
